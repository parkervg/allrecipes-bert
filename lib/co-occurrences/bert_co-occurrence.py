...
start_index = 1
df_chunk_1 = df[start_index : start_index + 25][
    df.columns[start_index : start_index + 50]
]
...
ax.text(
    18, -5, "Average Softmax across Verb/Noun Co-Occurence", fontsize=24, weight="bold"
)
ax.text(
    21, -4, "'now let me show you how to [MASK] the [MASK].'", fontsize=17, style="italic"
)  # Set ticks to all sides
ax.tick_params(
    right=True, top=True, labelright=True, labeltop=True, rotation=0, labelsize=16
)
# Rotate X ticks
plt.xticks(rotation="vertical")
# plt.show()
plt.savefig(
    "visualizations/bert_co-occurence_small1.png",
    dpi=500,
    bbox_inches="tight",
    pad_inches=0.5,
)
...
fig, ax = plt.subplots(figsize=dimensions)
ax = sns.heatmap(
    data=df, vmin=0, vmax=max_softmax + 0.1, cmap=cmap, linecolor="black", linewidths=0.01
)
ax.text(
    30, -8, "Average Softmax across Verb/Noun Co-Occurence", fontsize=20, weight="bold"
)
ax.text(
    34, -6, "'now let me show you how to [MASK] the [MASK].'", fontsize=13, style="italic"
)  # Set ticks to all sides
ax.tick_params(
    right=True, top=True, labelright=True, labeltop=True, rotation=0, labelsize=14
)
# Rotate X ticks
plt.xticks(rotation="vertical")
plt.show()
